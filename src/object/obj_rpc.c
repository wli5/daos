...
crt_proc_struct_obj_io_desc(crt_proc_t proc, struct obj_io_desc *oiod)
{
	...
	if (unlikely(rc))
		return rc;
	if (DECODING(proc_op) && oiod->oiod_nr > 0) {
	...
	if (oiod->oiod_siods != NULL) {
		rc = crt_proc_memcpy(proc, oiod->oiod_siods,
				   oiod->oiod_nr * sizeof(oiod->oiod_siods[0]));
		if (unlikely(rc)) {
			if (DECODING(proc_op))
...
crt_proc_struct_obj_iod_array(crt_proc_t proc, struct obj_iod_array *iod_array)
{
	...
	if (ENCODING(proc_op)) {
		...
		for (i = 0; i < off_nr; i++) {
			rc = crt_proc_uint64_t(proc, &iod_array->oia_offs[i]);
			if (unlikely(rc))
				return rc;
		}
				     ...
				     struct dtx_redundancy_group *drg)
{
	...
	rc = crt_proc_memcpy(proc, drg->drg_ids,
			     drg->drg_tgt_cnt * sizeof(drg->drg_ids[0]));
	if (unlikely(rc))
		return rc;
...
crt_proc_struct_dtx_memberships(crt_proc_t proc, struct dtx_memberships *mbs)
{
	...
	rc = crt_proc_memcpy(proc, mbs->dm_tgts,
			     mbs->dm_tgt_cnt * sizeof(mbs->dm_tgts[0]));
	if (unlikely(rc))
		return rc;
