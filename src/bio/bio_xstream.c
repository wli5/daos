...
struct bio_bdev {
	...
	struct spdk_bdev_desc	*bb_init_desc;
...
load_blobstore(struct bio_xs_context *ctxt, struct spdk_bdev_desc *desc,
...
destroy_bio_bdev(struct bio_bdev *d_bdev)
{
	...
	if (d_bdev->bb_init_desc != NULL) {
		spdk_bdev_close(d_bdev->bb_init_desc);
		d_bdev->bb_init_desc = NULL;
	}
...
put_bio_blobstore(struct bio_blobstore *bb, struct bio_xs_context *ctxt)
{
	...
	if (bs != NULL) {
		...
		D_ASSERT(bb->bb_desc != NULL);
		spdk_bdev_close(bb->bb_desc);
		bb->bb_desc = NULL;
	}
}
...
retry:
	...
	if (is_bbs_owner(ctxt, bbs)) {
		...
		/* Open bdev for SPDK blobstore */
		D_ASSERT(bbs->bb_desc == NULL);
		...
		bs = load_blobstore(ctxt, bbs->bb_desc, &d_bdev->bb_uuid,
				    false);
		if (bs == NULL) {
			rc = -DER_INVAL;
			goto out;
